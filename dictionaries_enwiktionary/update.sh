...
do
lynx -width=1000 -nolist -underscore -dump -assume_charset=utf-8 -display_charset=utf-8 -accept_all_cookies "http://en.wiktionary.org/w/index.php?title=$1-$letter&printable=yes" |\
awk '/::/ {gsub(/[\ ]+/, " "); gsub(/^[\ ]/, ""); print;}'>>$2 
sleep 10
done
}

for lang in es it pt fr nl de fi no sv cs hu pl ru ja arb cmn fa hi vi el he tr ko bg ro ca sh da is sw sq lt lv mk eo la id ms
do
...
for lang in es it fr fi pt de la nl pl ru cs sh mk hu el cmn da
do
